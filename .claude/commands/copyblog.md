...
```bash
node scripts/copyblog-images.mjs <slug> "<url1>" "<url2>" "<url3>" ...
```

- 인자: slug + 본문에 박을 이미지 URL 목록 (최대 8개)
- 출력: stdout 마지막 줄에 JSON 1개 — `{ ok, slug, bucket, images: [{ n, primary, all: {v1_flop, v2_crop_bright, ...} }], uploadOk, uploadFail }`
...
- 변형은 항상 **v1_flop (좌우반전)** 을 primary 로 사용 (가장 강력한 해시 차별화)
- 운영자가 다른 variant 를 쓰고 싶으면 `photos/output/copyblog/<slug>/` 에서 직접 선택
